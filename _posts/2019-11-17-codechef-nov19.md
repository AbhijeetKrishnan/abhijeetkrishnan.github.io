...
I tried to solve this problem for a while without success. I searched online and found [this](https://www.geeksforgeeks.org/find-smallest-range-containing-elements-from-k-lists/) GeeksforGeeks article describing exactly how to solve the problem using a min-heap. The idea is to use the min-heap to maintain a window of $M$ elements. We advance the window of $M$ elements by retrieving the minimum value element, which is compared against the maximum value element in the window. The maximum value in the window can be maintained separately, or by using a [min-max heap](https://cglab.ca/~morin/teaching/5408/refs/minmax.pdf) (see [this](https://www.codechef.com/viewsolution/27819250) AC solution which uses a min-max heap). The retrieved element is then replaced by the next element in that list. 
